        ...
        <form class="form-inline">
            ...
            <div class="form-group" style="margin-right: 10px;">
                <select name="status" class="form-control">
                    ...
                    <option value="1" {{ Request::get('status') == 1 ? "selected='selected'" : "" }}>In Progress
                    </option>
                    <option value="2" {{ Request::get('status') == 2 ? "selected='selected'" : "" }}>Out for Delivery
                    </option>
                    <option value="3" {{ Request::get('status') == 3 ? "selected='selected'" : "" }}>Delivered
                    </option>
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                @foreach($transactions as $transaction)
                    <tr>
                        <td scope="row" style="text-align: center;position:relative;" data-tooltip='Click here to view order detail' class="css-tooltip">
                            <a href="{{ route('get.user.order', $transaction->id) }}">DH{{ $transaction->id }}</a>
                        </td>
                        <td style="text-align: center">${{ number_format($transaction->tst_total_money,2,'.',',') }}</td>
                        ...
                        <td style="text-align: center">
                            @if (!in_array($transaction->tst_status , [-1,2,3]) )
                                <a href="{{ route('get.user.transaction.cancel',$transaction->id) }}" 
                                   class="btn-xs label-danger" style="color: white"><i class="fa fa-save"></i> Canceled</a>
                           @endif
                        </td>
                    </tr>
                @endforeach
...
@section('script')
    <div id="popup-transaction" class="modal text-center">
        ...
        <div class="footer">
            <a href="#" rel="modal:close" class="btn btn-pink ">Close</a>
