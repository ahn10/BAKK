                        ...
                        <tbody>
                            @foreach($shopping as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="js-total-item">${{ number_format($item->price * $item->qty,2,'.',',') }} </span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    <p style="float: right;margin-top: 20px;">Order Total: <b id="sub-total">${{ \Cart::subtotal(0) }} </b></p>
                </div>
            </div>
        </div>
        <div class="right">
                ...
                <div class="customer__content">
                    <form class="from_cart_register" action="{{ route('post.shopping.pay') }}" method="post">
                        ...
                        <div class="form-group">
                            <label for="name" >Full Name <span class="cRed">(*)</span></label>
                        ...
                        <div class="btn-buy">
                            <button class="buy1 btn btn-primary {{ \Auth::id() ? '' : 'js-show-login' }}" style="width: 100%;margin-top: 20px" type="submit" name="pay" value="online">
                            Place your order
                            </button>
                        </div>
                    </form>
